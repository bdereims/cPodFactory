...
case $BACKEND_NETWORK in
	"NSX-V")
		...
		;;
        "NSX-T")
		curl -s -k -u ${NSX_ADMIN}:${NSX_PASSWD} -X PUT -H "Content-Type: application/json" --data  "{\"display_name\":\"${2}\",\"transport_zone_path\":\"/infra/sites/default/enforcement-points/default/transport-zones/edc7dda0-5d70-489b-9e09-327654ee3ea3\"}" https://${NSX}/policy/api/v1/infra/segments/${2} 2>&1 > /dev/null
		curl -s -k -u ${NSX_ADMIN}:${NSX_PASSWD} -X PUT -H "Content-Type: application/json" -d "{\"spoofguard_profile_path\":\"/infra/spoofguard-profiles/default-spoofguard-profile\",\"segment_security_profile_path\":\"/infra/segment-security-profiles/new-segment-security-profile\",\"resource_type\":\"SegmentSecurityProfileBindingMap\"}" https://${NSX}/policy/api/v1/infra/segments/${2}/segment-security-profile-binding-maps/segment_security_binding_map_${2} 2>&1 > /dev/null
		curl -s -k -u ${NSX_ADMIN}:${NSX_PASSWD} -X PUT -H "Content-Type: application/json" -d "{\"mac_discovery_profile_path\":\"/infra/mac-discovery-profiles/new-mac-discovery-profile\",\"ip_discovery_profile_path\":\"/infra/ip-discovery-profiles/new-ip-discovery\",\"resource_type\":\"SegmentDiscoveryProfileBindingMap\"}" https://${NSX}/policy/api/v1/infra/segments/${2}/segment-discovery-profile-binding-maps/segment_discovery_binding_map_${2} 2>&1 > /dev/null
                ;;
esac
